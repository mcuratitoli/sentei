...
- **Distribuzione beta — TestFlight.** Build **`1.0.0+2`** approvata in Beta Review ed esterna. **🆕 Build `1.0.0+3` (2 lug 2026) COMPILATA e IMPACCHETTATA** (`build/ios/ipa/sentei.ipa`, 41 MB, firmata team `W8XCSNY6V3`, deployment target 14.0, tutte le validazioni ok) con l'estetica iOS + vista satellite + info punto — **da testare su device e caricare su TestFlight** (Transporter → gruppo "Amici"; il gruppo è già approvato → nessuna nuova Beta Review). Comando usato: `flutter build ipa --release --dart-define=MAPBOX_TOKEN=pk... --dart-define=GOOGLE_CLIENT_ID=...`. Guide: `docs/testflight-setup.md` (setup) + `docs/testflight-amici.md` (passo passo: upload + gruppo esterno + invito amici). Privacy policy pubblicata su **GitHub Pages** (`https://mcuratitoli.github.io/sentei/privacy-policy.html`, sorgente `docs/privacy-policy.html`); repo reso **pubblico**. Nota build iOS: progetto su **SPM** (non CocoaPods); se `flutter build ipa` fallisce con `...xcframework.zip already exists` → `rm -rf ~/Library/Caches/org.swift.swiftpm` + `SourcePackages`. Upload futuri: incrementare `version: 1.0.0+N` in `pubspec.yaml`, ribuildare, caricare via Transporter, assegnare al gruppo "Amici".
...
- **[~] Drive su Android (in corso, 4 lug 2026):** **lato codice fatto** — il provider cloud è ora **per-piattaforma**: su **Android** solo **Google Drive** (iCloud nascosto, `cloudProviderProvider` forza `googleDrive`), su **iOS** selettore **iCloud (default, a sinistra) · Google Drive**. `cloudServiceProvider` legge già `GOOGLE_SERVER_CLIENT_ID`. **Restano passi manuali/ambiente:** (1) **ripristinare la toolchain Android** su questo Mac (ora **JDK e Android SDK assenti** → `flutter doctor` ko; vedi `docs/android-apk-setup.md`); (2) creare in Google Cloud un **client OAuth Android** (package `com.mattiacuratitoli.sentei` + **SHA-1** debug) e un **client OAuth Web** (→ `GOOGLE_SERVER_CLIENT_ID`); (3) buildare con `--dart-define=GOOGLE_SERVER_CLIENT_ID=...`. Checklist completa in **`docs/cloud-google-drive-setup.md` §5**.
