...
- **[ ] Grado di difficoltà CAI (`cai_scale`) nella card della traccia disegnata (aperto, analisi fatta):** *Obiettivo utente:* sotto la riga/banda dei segnavia nel grafico altimetrico della card espansa, una **seconda riga che indica la scala CAI** (T/E/EE/EEA) in corrispondenza dei punti del percorso. *Fattibilità: alta, effort medio.* Il dato `cai_scale` è un **tag della relazione** già disponibile da entrambe le fonti (OSM2CAI: proprietà `cai_scale`; Overpass: `tags.cai_scale`, oggi ignorato). **Intuizione chiave:** è un attributo della *stessa relazione* già abbinata a ogni tratto da `trailSegmentsAlong` → la riga difficoltà avrebbe **gli stessi confini** della banda segnavia, riusando la segmentazione esistente (nessun nuovo matching). *Modifiche (additive, retro-compatibili):* (1) `TrailSegment` + `caiScale` nullable, `TrailRelation` + `caiScale`; (2) refactor `_nearestRef` da "ritorna ref" a "ritorna relazione abbinata" (**punto delicato**) + lettura del campo nelle due `fetchRelations`; (3) `track_codec.dart` campo `'sc'` con lettura retro-compatibile (tracce vecchie → `null`, scale ricalcolata al prossimo ricalcolo percorso); (4) `_ProfilePainter` seconda banda colorata per difficoltà (T verde/E blu/EE arancio/EEA nero) + `chartH` ridotto di `2*_bandHeight`. *UX:* due bande stringono il grafico (150px) → banda scale più sottile o mostrata solo se la scale è nota; serve mini-legenda T/E/EE/EEA; tratti senza `cai_scale` restano neutri (copertura parziale del tag). Bonus collegato all'item OSM2CAI (`cai_scale` → chip difficoltà).
