...
> 🆕 **[ ] Selettore tipo mappa (mappa / satellite / …) — DA FARE:** il tasto "livelli" in barra (passo 9) è già presente ma inerte. In futuro aprirà la scelta dello **stile mappa**: Outdoors (attuale) / satellite / ibrido / eventuali altri. Mapbox offre stili satellite (`mapbox://styles/mapbox/satellite-streets-v12`) commutabili via `styleUri`; serve gestire il re-setup di terreno/hillshade/layer sentieri al cambio stile (oggi in `_styleSetup`, che va rieseguito `onStyleLoaded`). UI possibile: action sheet Cupertino o mini-pannello a scelte con anteprime.
