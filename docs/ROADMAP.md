...
- **[ ] Info punto in esplorazione (mini-card altitudine + coordinate) — ANALIZZATA (1 lug 2026, da implementare):** *Obiettivo:* fuori dal disegno, toccando un punto qualsiasi della mappa (dove non c'è una traccia da selezionare) compare una **mini-card** in stile "nuova traccia" con **coordinate** e **altitudine** del punto. *Aggancio:* `_onTap` in `map_gl_screen.dart` — ramo esplorazione (`!state.drawing`): oggi `_selectNearest` fa `deselect()` se nessuna traccia entro ~22px; lì, invece di limitarsi a deselezionare, si popola un nuovo stato "punto ispezionato". *Dati:* coordinate già disponibili dal tap (`ll.LatLng`, formattare in decimale e/o gradi-primi-secondi); **altitudine** via `elevationServiceProvider` (`ElevationService.elevationAt(LatLng) → Future<double?>`, Terrarium con cache → funziona anche **offline** sulle aree scaricate, `null`/spinner mentre risolve o se tile assente). *UI:* nuovo `inspectedPointProvider` (StateProvider/Notifier con il punto o `null`); nel blocco in basso aggiungere un terzo stato oltre a barra e card-traccia (una `_PointInfoCard` `GlassSurface` con coordinate + quota + tasto chiudi); un tap sulla card-traccia o su una traccia esistente lo azzera. *Extra opzionali:* pin temporaneo (CircleAnnotation "dropped pin") sul punto; tasto "copia coordinate"; bottone "crea traccia da qui". *Vincoli:* la card non deve confliggere con `showCard` (traccia selezionata) — priorità: traccia selezionata > punto ispezionato > barra; azzerare il punto quando si entra in disegno o si seleziona una traccia.
