...
- **[~] Segnavia CAI da OSM2CAI (in implementazione):** integrazione del catasto ufficiale **REI/INFOMONT** (CAI + Wikimedia Italia, open ODbL) per i numeri sentiero, al posto/affianco di Overpass. Indagine API completa in **`docs/osm2cai-investigation.md`** (endpoint, formato bbox, parametro `sda`=accatastamento, campi risposta `ref`/`ref_REI`/`osmc_symbol`/`cai_scale`). Strategia: **`Osm2CaiTrailService` primario** (Italia) + **Overpass fallback** (confine FR/CH, o servizio giù), stesso pattern del geocoding Nominatim+Mapbox. Risolve i segnavia mancanti dove il tag `ref` OSM grezzo è assente ma il sentiero è accatastato CAI (es. Valle d'Aosta). Bonus futuri: `osmc_symbol` → segnavia colorati, `cai_scale` → chip difficoltà.
