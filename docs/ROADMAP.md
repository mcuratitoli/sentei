...
**UI mappa — riordino controlli + ricerca luoghi (giu 2026, da tap-testare su device):** barra in basso ora **lente · occhio · + · lista · impostazioni**; i bottoni **posizione** e **2D/3D** spostati **in alto a destra** sotto la bussola Mapbox (`_SideControls`/`_RoundMapButton`, ~44px, stile coordinato; bussola riposizionata sotto la safe area in `_configureOrnaments`). Nuova **ricerca luoghi** dalla lente: `GeocodingService` (Mapbox Geocoding v6, riusa il token `pk`) + `geocodingServiceProvider`; pannello `_SearchPanel` (campo testo + risultati, debounce 350ms, proximity = centro mappa) → al tap/invio la mappa vola sul luogo. **Focus traccia dalla lista:** selezionando una traccia in "Tracciati" la mappa la inquadra (`mapFocusProvider` + `_focusTrack` con `cameraForCoordinates`). Verificato: analyze pulito, 59 test verdi, layout ok sul simulatore; **da provare con tap** la digitazione ricerca e il focus.
...
- **Distribuzione beta — TestFlight (prima build CARICATA, 25 giu 2026):** `docs/testflight-setup.md`. Build `1.0.0+1` firmata (`build/ios/ipa/sentei.ipa`, 41 MB) e **caricata via Transporter** → "processing" su App Store Connect. Nota build: il progetto iOS usa **Swift Package Manager** (non CocoaPods); un primo tentativo è fallito per **cache SPM corrotta** (`...xcframework.zip already exists in file system` + blip di rete) → fix: `rm -rf ~/Library/Caches/org.swift.swiftpm` + `SourcePackages` in DerivedData, poi ribuild ok. Secret token Mapbox `sk` in `~/.netrc` (questo Mac configurato). **Prossimo:** creare gruppo tester esterni + Test Information su App Store Connect (prima build esterni → Beta App Review). Upload futuri: incrementare `version: 1.0.0+N` in `pubspec.yaml`.
- **Setup su nuovo Mac:** `docs/dev-setup.md` (Flutter 3.44.2, Xcode, CocoaPods, segreti Mapbox `sk` in `~/.netrc`, firma iOS).
- **Rimandati:** **bundling font** come asset (ora `google_fonts` scarica a runtime); **registrazione traccia live** (background location, Fase 2); **Drive su Android** (client OAuth + SHA-1).
