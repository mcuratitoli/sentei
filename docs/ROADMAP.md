...
- **Distribuzione beta — TestFlight (guida pronta, da eseguire):** `docs/testflight-setup.md`. Apple Developer attivo, `ITSAppUsesNonExemptEncryption=false`, versione `1.0.0+1`. Prossimo passo concreto: `flutter build ipa --release` con i `--dart-define`, poi upload via Transporter.
