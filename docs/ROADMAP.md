...
> 🆕 **[x] Selettore tipo mappa (Mappa / Satellite) — FATTO (2 lug 2026, da tap-testare su device):** il tasto "livelli" in barra apre un **action sheet Cupertino** con **Mappa** (Outdoors) e **Satellite** (`satellite-streets-v12`), con spunta sulla vista attiva (`MapStyleChoice`, `_onLayers`/`_setStyle`). Il cambio vista è **imperativo** (`map.loadStyleURI`, prop del MapWidget fisso → niente ricariche doppie) e **ri-esegue il setup** a ogni `onStyleLoaded` (`_runSetup`/`_settingUp` al posto del vecchio latch `_didSetup`): terreno 3D, sky, layer sentieri CAI e manager annotation vengono ricreati; l'**hillshade extra è saltato in satellite** (l'ortofoto ha già il rilievo); la **cache bbox dei numeri sentiero si azzera** così le etichette si ripopolano senza pan; la **centratura GPS iniziale** gira solo alla prima apertura (`_postSetupOnce`), non a ogni switch. *Follow-up possibili:* vista "ibrida", anteprime nel selettore, persistenza dell'ultima vista scelta.
