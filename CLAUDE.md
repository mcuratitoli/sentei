...
- **Numeri sentiero (ref CAI) e difficoltà:** non disponibili da BRouter → `TrailService` (`data/trails/`), strategia combinata: **OSM2CAI** primario (Italia, `ref` CAI/REI anche dove manca il tag OSM, es. Valle d'Aosta) → **Overpass** fallback (confini FR/CH). Risultato: un chip di difficoltà **per ogni grado presente** lungo la traccia (`presentCaiScales`, non solo il massimo) + banda per-tratto (`TrailSegment`, `cai_scale` T/E/EE/EEA, più `EEA:F` = via ferrata gestito come grado a sé), e, **sulla traccia selezionata**, il **colore** della linea sulla mappa segue il grado tratto per tratto (`caiScaleColor`: T verde / E teal / EE arancio / EEA rosso, grigio se ignoto). Le tracce non selezionate restano del proprio colore. Reso con 2 `LineLayer` su sorgente GeoJSON (`_savedTracksSourceId` in `map_gl_screen.dart`) — linea sempre **unita** col fade bianco (`line-border`), colore risolto in Dart per feature; non `PolylineAnnotationManager` (rende le linee peggio).
