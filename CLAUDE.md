...
- **Numeri sentiero (ref CAI) e difficoltà:** non disponibili da BRouter → `TrailService` (`data/trails/`), strategia combinata: **OSM2CAI** primario (Italia, `ref` CAI/REI anche dove manca il tag OSM, es. Valle d'Aosta) → **Overpass** fallback (confini FR/CH). Risultato: un chip di difficoltà **per ogni grado presente** lungo la traccia (`presentCaiScales`, non solo il massimo) + banda per-tratto (`TrailSegment`, `cai_scale` T/E/EE/EEA, più `EEA:F` = via ferrata gestito come grado a sé), e **tratteggio della linea del tracciato sulla mappa** in base al grado (piena T / trattini E / trattini corti EE / tratto-punto EEA — `caiScaleDash` in `ui/cai_difficulty.dart`, fonte condivisa con la legenda). Reso con **`LineLayer` su sorgente GeoJSON** (`_savedTracksSourceId` in `map_gl_screen.dart`), un layer per grado filtrato su una proprietà `g` — non `PolylineAnnotationManager`, che rende il `line-dasharray` sgranato.
